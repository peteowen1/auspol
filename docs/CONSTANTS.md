...
| Constant | Where | What it is | Status |
|---|---|---|---|
| `SEAT_SWING_COEF` = fed **+0.708**, retirement **−1.396**, soph_cand **+2.559**, soph_party **+1.609** | `seat_swing.R` | How far a seat departs from the statewide two-party swing, given the anchor's transposed federal swing and its retirement/sophomore flags | **ESTIMATED from data.** Fitted on all 180 seats of Victoria 2022 and NSW 2023 by `scripts/test_seat_swing_predictors.R`. Validated leave-one-election-out: out-of-sample MAE 3.948 → 3.425, a gain of 0.523 against a pre-registered bar of 0.10, positive in BOTH held-out elections. Every sign was required in advance (`plans/prereg-seat-swing-predictors.md`); `soph_party` is not significant (t = 1.2) and is kept because the model was fixed before fitting. |
