...
| `parties_in(n = 8)` / `cnt >= 8` | `fit_vic.R:115,183`, `fit_nsw.R:84,165`; `>= 25` federal | **Which parties exist in the forecast at all** | **MISFILED HERE — see below.** This is not an input-sanity guard. A party under the floor is not fitted, so its vote stays inside `OTH` and no fold correction runs on it. One Nation has 7 polls in NSW 2023 against a floor of 8 and is excluded on that one poll; the resulting `OTH` mixes firms that break the party out with firms that do not, and is fitted at 15.30 against an actual of 17.96. Whether the floor is right has never been tested — see `plans/prereg-party-inclusion-floor.md`. |
