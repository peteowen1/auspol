...
| `PARTY_INCLUSION_FLOOR` (`scales.R`) | `fit_vic.R:184,196`, `fit_nsw.R:164,177`; `>= 25` federal | **Which parties exist in the forecast at all** — not an input-sanity guard, which is how it was filed until 2026-08-19 and why nobody looked at it. A party under the floor is not fitted, so its vote stays inside `OTH`. Lowering it is monotonically worse (tested 2026-08-19). **Raised 8 → 15 on 2026-08-24** (`plans/prereg-inclusion-floor-15-adoption.md`): floor 15 beats 8 by 0.061 MAE, three times the adoption bar — refused on 2026-08-19 because it drops One Nation from NSW 2027 (21.0% on 8 polls), a refusal criterion added AFTER the result. Re-opened and adopted with that anchor now pre-registered and its cost disclosed: Victoria 2026, the only forecast this repo publishes, is unaffected (ONP clears 15 by 4 polls); NSW 2027 is validation infrastructure, not a live forecast, and its ONP now folds into `OTH` (confirmed: `OTH` rises to 35.4%). `scripts/test_inclusion_floor.R`'s anchor check (`IF6`) stays wired in unweakened for any future change. `fit_vic.R:115`'s separate `parties_in(n = 8)` default (validation-cycle noise-factor estimation, not the live party set) is a distinct constant, deliberately untouched. `scripts/test_anchor_k.R:46` and `scripts/test_fp_coverage.R:43` copy the old `cnt >= 8` pattern for one-off diagnostics on closed experiments, not the live pipeline — also deliberately left at 8, not silently updated. |
