...
| Constant | Where | What it is | Status |
|---|---|---|---|
| `POLL_TRACKING_BOUND = 2.5` | `poll_tracking.R` | Maximum permitted |fitted endpoint − mean of the final 90 days of polls|, asserted by `L3`/`FL3`/`NL3` | **ESTIMATED from data.** The 99th percentile of that quantity over 138 party-cycles with complete actuals, rounded up to 0.5 (percentile 2.478). The *rule* was committed in `plans/prereg-per-party-poll-check.md` before the number was computed, with a refusal condition above 5.0. Replaced a `sum = 100 ± 5` check the model does not promise. |
