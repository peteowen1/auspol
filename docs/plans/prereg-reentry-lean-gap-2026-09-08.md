...
## Pooled outcome, 2026-09-08

Measured on all 22 pairs and 2,050 seat-elections after three bugs were fixed
(commits `d17c8dd` and `e2006a0`); the first sweep was discarded because
`apply_reentry_prior()` was silently falling back to the flat ratio.

| clause | requirement | result | verdict |
|---|---|---|---|
| 1 | zero predictions above 40 | 3 (one on the GLM path) | **fail** |
| 2 | PB3f at least as good as prior OFF's 0.3149 | **0.3066** | pass |
| 3 | no jurisdiction beyond 2 SE of its own paired difference | all six inside | pass |

Per jurisdiction, arm D against prior OFF. Log loss, lower is better; `SE` is
the standard error of the paired per-seat difference, and the tolerance is
twice it.

| region | n | off | arm D | diff | SE | in SEs | verdict |
|---|--:|--:|--:|--:|--:|--:|---|
| fed | 1036 | 0.3295 | 0.3270 | −0.0024 | 0.0021 | −1.17 | pass |
| nsw | 181 | 0.3666 | 0.3858 | +0.0192 | 0.0171 | +1.12 | pass |
| qld | 186 | 0.3274 | 0.3300 | +0.0026 | 0.0066 | +0.39 | pass |
| sa | 47 | 0.3976 | **0.3924** | −0.0052 | 0.0300 | −0.17 | pass |
| vic | 239 | 0.2676 | 0.2683 | +0.0007 | 0.0015 | +0.43 | pass |
| wa | 361 | 0.4109 | **0.3973** | −0.0136 | 0.0544 | −0.25 | pass |

South Australia now **beats** the prior-off baseline, having been one of the two
jurisdictions that refused the original prior.

### The effect is not statistically distinguishable from zero

On the 2,044 seats neither arm floors, PB3f moves −0.0038 with a paired SE of
0.0044 — **0.85 SE**. Pooled over all 2,050 seats the move is −0.0017 at 0.18
SE. Reported here rather than buried, because the per-jurisdiction table above
reads as a win and the pooled test says the evidence is thin.

### The floor churns rather than shrinking

| pair | seat | off | arm D | |
|---|---|--:|--:|---|
| wa2001 | **Kimberley** | 1e-06 | **0.51** | **off the floor** |
| wa2001 | Pilbara | 5.8e-03 | 1e-06 | onto the floor |
| wa2005 | Alfred Cove | 5.4e-03 | 1e-06 | onto the floor |
| fed2013 | Fairfax | 1e-06 | 1e-06 | unchanged |
| nsw2019 | Orange | 1e-06 | 1e-06 | unchanged |
| wa2008 | Kalgoorlie | 1e-06 | 1e-06 | unchanged |

**Kimberley is the seat this entire line of work exists for.** Carol Martin won
it with 42.2% where Labor had not stood, the model gave her 0.000000, and it
now gives her **0.51**. That is the single largest per-seat improvement in the
sequence.

Against it, two seats move onto the floor, and both are the **flat-ratio path
for the major parties** — the thing this plan's limitations section named in
advance as out of scope and "the second-largest source of tail error". Alfred
Cove gets Labor at 41.9 against an actual 22.8, which pushes the independent
who actually won to 1e-06.

### Verdict

Clause 1 refuses the arm, and it is not rewritten. Independently of that, the
primary's effect is 0.85 SE, so the pooled evidence would not support shipping
even if clause 1 had been drafted better.

What the sequence has produced is nonetheless real and is kept: the prior no
longer swings twice, no longer predicts 74.3% for a party polling 6.8%, no
longer silently falls back to the flat ratio, and Western Australia's covariate
model works at all for the first time. `AUSPOL_REENTRY` stays 0.

**The next experiment is named by the evidence rather than chosen:** Labor and
the Coalition fall back to a flat ratio of 1.0 on 2 and 4 rows, and that
fallback now owns both new floor seats and the largest remaining prediction
(Churchlands, 53.2 against an actual 59.0). It is the binding constraint on the
whole prior and it needs its own plan.
