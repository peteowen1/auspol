# auspol build journal — 2026-08-19 to 2026-08-23

(Session write-ups moved out of the hub. Nothing here is open work.)

Moved verbatim out of `docs/NEXT-STEPS.md` on 2026-09-04, newest first: the
Victoria 2026 target snapshot as it stood on 2026-08-23 (its seat-forecast
numbers are stale by construction and were already marked "verify freshness
against a rerun before quoting these"), then the four dated session
write-ups from 2026-08-19 through 2026-08-22 in the order they already
carried in the hub. Their conclusions live in `reviews/`, `CONSTANTS.md` and
the code; this file holds the narrative of how they got there.

**Items pulled forward as still-live before this move — see
`docs/NEXT-STEPS.md`'s `Awaiting Pete` section, not this file**: the Centre
Alliance/NXT/SA-BEST party classification call, the WA survivor-multiset
matrix test, and the independent-emergence exogenous-input decision (named
list / seat polls / market odds).
