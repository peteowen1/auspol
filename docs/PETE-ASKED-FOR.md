...
| follow-on I ran unprompted: is the Victorian gap getting worse? | **No — converging.** Refitting at each ONP poll date: −3.97 at 8 polls, −4.13 at 15, −2.85 at 19. It has breached at every cutoff since January, so `S7` catches a year-old condition rather than a new fault. At 6–7 polls One Nation was dropped from the published fit entirely (`min_polls = 8`). |
