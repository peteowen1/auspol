...
| ask | status |
|---|---|
| *"lets correct/adjust using ALL demo data ... glmnet can help give each one a coefficient right?"* | **BUILT, OFF (`AUSPOL_DEMO_RESID=0`), and he was told why in a message.** All seven census columns under a leave-one-pair-out elastic net, all six harnesses, 22 pairs. Pooled seat log loss 0.2849 -> 0.2831 (-0.0018, t = -1.83, binomial p = 0.143). REFUSED on the pre-registered condition that qld2024 must not worsen; it worsened +0.0024, which the permutation control puts ~27 SDs outside the null. `docs/plans/prereg-demographic-axis-2026-09-15.md`. |
...
| **his ask: seat-seat correlation in the simulation — "if two regions with very similar demographics, if one of them swings to ONP then the other one is way more likely too ... as a result for a seat comes in in a sim it should slightly change the probabilities in all other seats"** | **NOT DONE, and it is a real gap in the published forecast.** `src/seat_sim_core.cpp:88` draws each seat's deviation as `R::rnorm(0, sdc[k])`, independent across seats. There IS a shared statewide `shift[k]` with a Cholesky for PARTY-party correlation, and there is NO seat-seat term at all. Consequence: the seat-COUNT distribution is too narrow even when each seat's marginal probability is right. |
