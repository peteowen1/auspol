...
# Points-scale here; the logit-scale equivalents live in test-scales.R.
...
test_that("estimate_trend_sigmas recovers known sigmas on the logit scale", {
  syn <- make_synthetic_logit(seed = 17, n_polls = 400, T_days = 700,
                              start_pct = 10, sigma_obs = 0.12, sigma_rw = 0.008)
  est <- estimate_trend_sigmas(list(syn$polls), "ALP", prior_results = 10)

  expect_equal(est$convergence, 0)
  expect_false(est$at_bound)
  expect_gt(est$sigma_obs, 0.12 * 0.7); expect_lt(est$sigma_obs, 0.12 * 1.3)
  expect_gt(est$sigma_rw, 0.008 * 0.6); expect_lt(est$sigma_rw, 0.008 * 1.4)
})
...
test_that("logml prefers the generating sigmas over badly wrong ones", {
  ...
  prep <- prep_trend_obs(syn$polls, "ALP", 3, "points", 35)
