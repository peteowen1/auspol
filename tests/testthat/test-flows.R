# Preference-flow SELECTION: which recorded estimate flows_for() picks, and
# how it carries one forward when the requested year has none. The anchor's
# file is hand-maintained and incomplete; without carry-forward a missing year
# silently becomes a 50-50 split.
#
# Every call here passes `estimate = FALSE`. That is not a convenience: these
# tests assert the CARRIED value (81.94, 79.0, 77.0), and estimation would
# legitimately replace it with a figure fitted from the record, so leaving it
# on would test two things at once and fail for the right reason in a way that
# looks like the wrong one. Estimation has its own file, test-flow-model.R.
# It also keeps these tests free of the anchor data: estimation needs the
# election calendar to know which elections have been held, and these fixtures
# are synthetic.
...
test_that("flows_for actually filters by region and year (NSE regression)", {
  ...
  out <- flows_for(f, 2026, "vic", quiet = TRUE, estimate = FALSE)
...
test_that("flows_for carries the most recent estimate forward", {
  f <- fake_flows()
  expect_message(flows_for(f, 2026, "vic", estimate = FALSE), "carried forward")
  out <- flows_for(f, 2026, "vic", quiet = TRUE, estimate = FALSE)
...
test_that("flows_for prefers the nearer year when several exist", {
  f <- fake_flows()
  out <- flows_for(f, 2027, "nsw", quiet = TRUE, estimate = FALSE)
...
test_that("flows_for never looks past the requested year or across regions", {
  f <- fake_flows()
  out <- flows_for(f, 2020, "nsw", quiet = TRUE, estimate = FALSE)
  expect_equal(out$flow_alp[out$party == "GRN"], 77.0)   # the 2018 estimate
  expect_false("ONP" %in% out$party)                      # ONP is vic-only here
  expect_error(flows_for(f, 2026, "qld", estimate = FALSE), "No preference flows")
})
...
test_that("an exact-year match is silent and unchanged", {
  f <- fake_flows()
  expect_silent(out <- flows_for(f, 2018, "vic", estimate = FALSE))
  expect_true(all(out$flow_year == 2018))
})
