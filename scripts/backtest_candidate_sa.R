...
if (identical(Sys.getenv("AUSPOL_MINOR_DEFECT_BASE_PRED", "0"), "1")) {
  ...
  if (is.null(.mfd$discount)) {
    ...
    # TWO-RATE: sitting-member switcher keeps .minor_disc (renamed here from
    # discount_mp for the same reason major_discount does); a non-sitting
    # switcher gets discount_loser instead. docs/reviews/
    # minor-defector-two-rate-2026-09-17.md.
    ...
    if (!is.null(.mfd$discount_loser) && is.finite(.mfd$discount_loser)) {
      ...
      cat(sprintf("BS0n  two-rate: sitting-member %.3f, non-sitting %.3f\n", .minor_disc, .minor_disc_loser))
    }
  }
}
