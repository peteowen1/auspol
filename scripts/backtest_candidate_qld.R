...
st_b <- forecast_statewide_or_oracle("qld", PAIR$to, PAIR$asof, colnames(mat), st_a, st_b,
                                     code = "BQ0", n_sims = N_SIMS, seed = as.integer(Sys.getenv("AUSPOL_SEED", "42")))
...
if (identical(Sys.getenv("AUSPOL_MINOR_DEFECT_BASE_PRED", "0"), "1")) {
  ...
  if (is.null(.mfd$discount)) {
    ...
    if (!is.null(.mfd$discount_loser) && is.finite(.mfd$discount_loser)) {
      ...
      cat(sprintf("BQ0n  two-rate: sitting-member NO DISCOUNT, non-sitting %.3f, unknown-status %.3f\n", .minor_disc_loser, .minor_disc))
    }
  }
}
...
if (identical(Sys.getenv("AUSPOL_SALIENCE_SURGE_V2", "0"), "1")) {
  ...
  if (!is.null(hz)) {
    ...
    if (identical(Sys.getenv("AUSPOL_SALIENCE_EXP_SD", "0"), "1")) {
      ...
      cat(sprintf("BQ0d salience sd override: %d of %d cells set\n",
